...
(require "cbneednew-deref-CK.rkt")
...
(define-metafunction λneed
  ...
  [(get-bindings (λ x e) ((x_1 e_1) ...))
   ((x_1 e_1) ...)])
(define (AF-CK-equiv? t)
  (printf "~a) checking: ~a\n" n t)
  (set! n (add1 n))
  (let ([res1 (apply-red-no-ambig* λ-need-red-alpha t)]
        [res2 (term (φ ,(apply-red-no-ambig* red-ck (term (,t (mt))))))])
    (and (redex-match λ-need A res1)
         (redex-match λneed a res2)
         (let ([res3 (apply-red-no-ambig* λ-need-red-nobeta res2)])
           (and (redex-match λ-need A res3)
                (term (alpha-equiv? ,res1 ,res3)))))))
...
(define-syntax (redex-check-go stx)
  (syntax-case stx ()
    [(_) #'(redex-check λneed e (AF-CK-equiv? (term e)) 
                        #:prepare close 
                        #:attempts 1000000)]))
