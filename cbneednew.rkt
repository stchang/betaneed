...
(define-language λneed
  (e x (λ x e) (e e))
  (x (variable-except e λ v a A n hole any C B E A+ A-))
  (v (λ x e))
  (a (in-hole A v))
  (n integer)
  (C hole (λ x C) (C e))
  (Cbal (side-condition C_1 (and (term (balance C_1))
                                 (= 0 (term (balance C_1))))))
  (A+ hole ((in-hole A A+) e))
  (A- hole (in-hole A (λ x A-)))
  ...
  (A+new hole ((in-hole Anew A+new) e))
  (A-new hole (in-hole Anew (λ x A-new)))
  (Anew hole (side-condition 
              (in-hole A+new_1 ((in-hole Anew (λ x A-new_1)) e))
              (redex-match λneed Anew (term (in-hole A+new_1 A-new_1)))))
  (E hole (E e) (λ x E)
     (side-condition 
      (in-hole A+_1 ((in-hole A_1 (λ x_1 (in-hole A-_1 (in-hole E_1 x_1)))) E))
      (and (redex-match λneed A (term (in-hole A+_1 A-_1)))
           (not (member (term x_1) (term (bound-vars-C A-_1))))
   ...
   (--> 
    ...
    (where 
     (in-hole
      ...
      (in-hole 
       A+ 
       ((in-hole A_1 (λ x_1 (in-hole A-_1 (in-hole E_1 x_1))))
        a)))
     e_lhs)
    (where ((in-hole A_2 (λ x_2 (in-hole A-_2 (in-hole E_2 x_2))))
            (in-hole A_3 v))
           (rename-bound-vars
            ((in-hole A_1 (λ x_1 (in-hole A-_1 (in-hole E_1 x_1)))) a)
            (bound-vars e_lhs)))
    (where
     e_rhs
     (in-hole
      ...
      (in-hole 
       ...
       (in-hole 
        ...
        (in-hole 
         A_3
         (subst
          (in-hole A-_2 (in-hole E_2 x_2))
          x_2 v))))))
    (side-condition (and (redex-match λneed A (term (in-hole A+ A-_1)))
                         (redex-match λneed A (term (in-hole A+ A-_2)))))
    (side-condition (and (not (member (term x_1) (term (bound-vars-C A-_1))))
                         (not (member (term x_1) (term (bound-vars-E E_1))))
                         (not (member (term x_2) (term (bound-vars-C A-_2))))
...
(define n 0)
#;(redex-check λneed Cbal ((λ (c) 
                           (printf "~a) ~a\n" n c)
                           (set! n (add1 n))
                           (redex-match λneed A c)) (term Cbal))
             #:attempts 1000000)
(redex-check λneed A ((λ (a) 
                          (printf "~a) ~a\n" n a)
                          (set! n (add1 n))
                          (redex-match λneed Cbal a)) (term A)) 
               #:attempts 1000000)
